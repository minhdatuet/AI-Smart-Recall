    ...
    public class SimpleJsonInstance 
    {
        ...
        public class PlayFabSimpleJsonCuztomization : PocoJsonSerializerStrategy
        {
            ...
            public override object DeserializeObject(object value, Type type)
            {
                ...
                {
                    DateTime output;
                    var result = DateTime.TryParseExact(valueStr, _defaultDateTimeFormats, CultureInfo.InvariantCulture, DateTimeStyles, out output);
                    if (result)
                        return output;
                }
                else if (type == typeof(DateTimeOffset))
                {
                    ...
                    var result = DateTimeOffset.TryParseExact(valueStr, _defaultDateTimeFormats, CultureInfo.InvariantCulture, DateTimeStyles, out output);
                    if (result)
                        return output;
                }
        ...
        {
            return YoSimpleJson.DeserializeObject<T>(json, ApiSerializerStrategy);
        }

        public T DeserializeObject<T>(string json, object jsonSerializerStrategy)
        {
            return YoSimpleJson.DeserializeObject<T>(json, (IJsonSerializerStrategy)jsonSerializerStrategy);
        }

        public object DeserializeObject(string json)
        {
            return YoSimpleJson.DeserializeObject(json, typeof(object), ApiSerializerStrategy);
        }

        public string SerializeObject(object json)
        {
            return YoSimpleJson.SerializeObject(json, ApiSerializerStrategy);
        }

        public string SerializeObject(object json, object jsonSerializerStrategy)
        {
            return YoSimpleJson.SerializeObject(json, (IJsonSerializerStrategy)jsonSerializerStrategy);
        }
    }
}
